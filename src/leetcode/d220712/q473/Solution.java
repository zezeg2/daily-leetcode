...
//
    ...
    static boolean makesquare3(int[] matchsticks){
        ...
        int[] sides = new int[4];
        Integer[] sorted = Arrays.stream(matchsticks).boxed().toArray(Integer[]::new);
        Arrays.sort(sorted, Collections.reverseOrder());

        return backtrack(0, sorted,sides,sideLength);
    }

    static boolean backtrack(int index, Integer[] sorted, int[] sides, int sideLength){
        if (index == sorted.length) return true;
        for (int i = 0; i < 4; i++){
            if (sides[i] + sorted[index] <= sideLength){
                sides[i] += sorted[index];
                if (backtrack(index+1, sorted, sides, sideLength)){
                    return true;
                }
                sides[i] -= sorted[index];
            }
    ...
    public static void main(String[] args) {
        System.out.println(makesquare3(new int[]{1,1,2,2,2}));
        Queue<Integer> pq1 = new PriorityQueue<>(Collections.reverseOrder());
    }
}
