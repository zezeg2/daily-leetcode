...
public class Solution {
    ...
    public static boolean solution(int[][] key, int[][] lock) {
        ...
        for (int i = keyLength - 1; i < keyLength + lockLength - 1; i++) for (int j = keyLength - 1; j < keyLength + lockLength - 1; j++) {
            expanded[i][j] = lock[i - keyLength + 1][j - keyLength + 1];
        }
        for (int i = 0; i + keyLength <= expanded.length; i++) for (int j = 0; j + keyLength <= expanded.length; j++) {
